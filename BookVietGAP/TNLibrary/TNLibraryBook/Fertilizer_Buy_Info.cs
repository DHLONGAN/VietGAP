...
namespace TNLibrary.Book
{
    public class Fertilizer_Buy_Info
    {
        ...
        + " DatetimeBuy ,FertilizerKey ,Quantity ,Price ,CompanyKey ,Address ,MemberKey ,CooperativeKey ,SeedsKey ,UnitKey ,Total ,CreatedBy ,CreatedDateTime ,ModifiedBy ,ModifiedDateTime ,IsActive ,IsSync ) "
         + " VALUES ( "
         + "@DatetimeBuy ,@FertilizerKey ,@Quantity ,@Price ,@CompanyKey ,@Address ,@MemberKey ,@CooperativeKey ,@SeedsKey ,@UnitKey ,@Total ,@CreatedBy ,@CreatedDateTime ,@ModifiedBy ,@ModifiedDateTime ,@IsActive ,@IsSync ) ";
            string zResult = "";
            string zConnectionString = ConnectDataBase.ConnectionString;
