...
namespace TNLibrary.Book
{
    public class Fertilizer_Use_Info
    {
        ...
        + " DateTimeUse ,SeedKey ,Parcel ,Area ,FertilizerKey ,FormulaUsed ,Quantity ,Howtouse ,MemberKey ,CooperativeKey ,QuarantinePeriod ,UnitKey ,CreatedBy ,CreatedDateTime ,ModifiedBy ,ModifiedDateTime ,IsActive ,IsSync ) "
         + " VALUES ( "
         + "@DateTimeUse ,@SeedKey ,@Parcel ,@Area ,@FertilizerKey ,@FormulaUsed ,@Quantity ,@Howtouse ,@MemberKey ,@CooperativeKey ,@QuarantinePeriod ,@UnitKey ,@CreatedBy ,@CreatedDateTime ,@ModifiedBy ,@ModifiedDateTime ,@IsActive ,@IsSync ) ";
            string zResult = "";
            string zConnectionString = ConnectDataBase.ConnectionString;
