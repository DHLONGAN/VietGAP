...
namespace TNLibrary.Book
{
    public class HarvestedForSale_Info
    {
        ...
        + " Datetime ,Code ,QuantityHarvested ,QuantitySale ,WhereToBuy ,MemberKey ,SeedsKey ,UnitKey ,Total ,CreatedBy ,CreatedDateTime ,ModifiedBy ,ModifiedDateTime ,IsActive ,IsSync ) "
         + " VALUES ( "
         + "@Datetime ,@Code ,@QuantityHarvested ,@QuantitySale ,@WhereToBuy ,@MemberKey ,@SeedsKey ,@UnitKey ,@Total ,@CreatedBy ,@CreatedDateTime ,@ModifiedBy ,@ModifiedDateTime ,@IsActive ,@IsSync ) ";
            string zResult = "";
            string zConnectionString = ConnectDataBase.ConnectionString;
