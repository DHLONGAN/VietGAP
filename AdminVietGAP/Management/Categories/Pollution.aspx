<%@ Page Title="" Language="C#" MasterPageFile="~/SiteMain.Master" AutoEventWireup="true" CodeBehind="Pollution.aspx.cs" Inherits="Management.Categories.Pollution" %>
...
<asp:ScriptManager ID="ScriptManager1" runat="server">
     ...
     <div id="ListLayer" style="border: 0px solid red; width:100%; overflow: hidden; 
        position: absolute;">
        <div class="TitleForm">
            <font size=6>TÁC NHÂN Ô NHIỄM</font>
        </div>
        ...
        <div style="padding-right: 20px">
            <asp:GridView ID="GV_Pollution" runat="server" Width="100%" AutoGenerateColumns="false"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Tên gọi">
                        <ItemTemplate>
                            <div style="cursor: pointer" onclick="PollutionEdit('<%# Eval("PollutionKey")%>')">
                                <asp:Label ID="txtSeedsName" Text='<%# Eval("PollutionName")%>' runat="server"></asp:Label>
                            </div>
                        </ItemTemplate>
                    ...
                    <asp:TemplateField>
                    <HeaderTemplate>
                    <asp:Image ID="Image1" runat="server" ImageUrl="~\Img\Icons\add_one.png" style="cursor: pointer" onClick="PollutionEdit(0)"/>
                    </HeaderTemplate>
                        <ItemTemplate>
                            <asp:ImageButton ID="cmdDel" CommandName="ItemDel" OnCommand="GrDelete" ImageUrl="~\Img\Icons\Del.gif " runat="server" OnClientClick="javascript:return confirm('Bạn có chắc chắn xóa không?');"  CommandArgument='<%# Eval("PollutionKey")%>' />
                        </ItemTemplate>
                        <ItemStyle Width="20px" HorizontalAlign="Center" />
    ...
    <script type="text/javascript">
        ...
        function PollutionEdit(ID) {
            var oWnd = $find("<%= rwPesticide.ClientID %>");
            oWnd.show();
            oWnd.setUrl("Pollution_Edit.aspx?Key=" + ID);
            // oWnd.maximize();
