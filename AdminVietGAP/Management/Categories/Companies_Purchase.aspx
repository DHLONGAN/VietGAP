<%@ Page Title="" Language="C#" MasterPageFile="~/SiteMain.Master" AutoEventWireup="true" CodeBehind="Companies_Purchase.aspx.cs" Inherits="Management.Categories.PUL_Companies_Purchase" %>
...
<asp:ScriptManager ID="ScriptManager1" runat="server">
     ...
     <div id="ListLayer" style="border: 0px solid red; width:100%; overflow: hidden; 
        position: absolute;">
        <div class="TitleForm">
            <font size=6>CÔNG TY KINH DOANH - THU MUA</font>
        </div>
        ...
        <div style="padding-right: 20px">
            <asp:GridView ID="GV_Companie" runat="server" Width="100%" AutoGenerateColumns="false"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Tên Công ty">
                        <ItemTemplate>
                            <div style="cursor: pointer" onclick="Companies_PurchaseEdit('<%# Eval("CompanyKey")%>')">
                    ...
                    <asp:TemplateField HeaderText="Thành phố">
                        <ItemTemplate>
                            <asp:Label ID="txtCity" Text='<%# Eval("City")%>' runat="server"></asp:Label>
                        </ItemTemplate>
                        <ItemStyle Width="200px" HorizontalAlign="Left" />
                    </asp:TemplateField>
                     <asp:TemplateField HeaderText="Quốc gia">
                        <ItemTemplate>
                            <asp:Label ID="txtCountry" Text='<%# Eval("CountryName")%>' runat="server"></asp:Label>
                        </ItemTemplate>
                        <ItemStyle Width="200px" HorizontalAlign="Left" />
                    </asp:TemplateField>
                    <asp:TemplateField>
                    <HeaderTemplate>
                    <asp:Image ID="Image1" runat="server" ImageUrl="~\Img\Icons\add_one.png" style="cursor: pointer" onClick="Companies_PurchaseEdit(0)"/>
                    </HeaderTemplate>
                        <ItemTemplate>
                            <asp:ImageButton ID="cmdDel" CommandName="ItemDel" OnCommand="GrDelete" ImageUrl="~\Img\Icons\Del.gif " runat="server" OnClientClick="javascript:return confirm('Bạn có chắc chắn xóa không?');"  CommandArgument='<%# Eval("CompanyKey")%>' />
                        </ItemTemplate>
                        <ItemStyle Width="20px" HorizontalAlign="Center" />
    ...
    <script type="text/javascript">
        ...
        function Companies_PurchaseEdit(ID) {
            var oWnd = $find("<%= rwPesticide.ClientID %>");
            oWnd.show();
            oWnd.setUrl("Companies_Purchase_Edit.aspx?Key=" + ID);
            // oWnd.maximize();
