<%@ Page Title="" Language="C#" MasterPageFile="~/SiteMain.Master" AutoEventWireup="true" CodeBehind="CertifiedOrganization.aspx.cs" Inherits="Management.Categories.CertifiedOrganization" %>
...
<asp:ScriptManager ID="ScriptManager1" runat="server">
     ...
     <div id="ListLayer" style="border: 0px solid red; width:100%; overflow: hidden; 
        position: absolute;">
        <div class="TitleForm">
            <font size=6>TỔ CHỨC CHỨNG NHẬN</font>
        </div>
        ...
        <div style="padding-right: 20px">
            <asp:GridView ID="GV_CertifiedOrganization" runat="server" Width="100%" AutoGenerateColumns="false"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Tên Công ty">
                        <ItemTemplate>
                            <div style="cursor: pointer" onclick="CertifiedOrganizationEdit('<%# Eval("CertifiedOrganization_Key")%>')">
                                <asp:Label ID="txtCompanyName" Text='<%# Eval("CertifiedOrganization_Name")%>' runat="server"></asp:Label>
                            </div>
                        </ItemTemplate>
                    ...
                    <asp:TemplateField HeaderText="Số điện thoại">
                        <ItemTemplate>
                            <asp:Label ID="txtCity" Text='<%# Eval("Phone")%>' runat="server"></asp:Label>
                        </ItemTemplate>
                        <ItemStyle Width="200px" HorizontalAlign="Left" />
                    </asp:TemplateField>
                     <asp:TemplateField HeaderText="Email">
                        <ItemTemplate>
                            <asp:Label ID="txtCountry" Text='<%# Eval("Email")%>' runat="server"></asp:Label>
                        </ItemTemplate>
                        <ItemStyle Width="200px" HorizontalAlign="Left" />
                    </asp:TemplateField>
                    <asp:TemplateField>
                    <HeaderTemplate>
                    <asp:Image ID="Image1" runat="server" ImageUrl="~\Img\Icons\add_one.png" style="cursor: pointer" onClick="CertifiedOrganizationEdit(0)"/>
                    </HeaderTemplate>
                        <ItemTemplate>
                            <asp:ImageButton ID="cmdDel" CommandName="ItemDel" OnCommand="GrDelete" ImageUrl="~\Img\Icons\Del.gif " runat="server" OnClientClick="javascript:return confirm('Bạn có chắc chắn xóa không?');"  CommandArgument='<%# Eval("CertifiedOrganization_Key")%>' />
                        </ItemTemplate>
                        <ItemStyle Width="20px" HorizontalAlign="Center" />
    ...
    <script type="text/javascript">
        ...
        function CertifiedOrganizationEdit(ID) {
            var oWnd = $find("<%= rwPesticide.ClientID %>");
            oWnd.show();
            oWnd.setUrl("CertifiedOrganization_Edit.aspx?Key=" + ID);
            // oWnd.maximize();
            oWnd.setSize(700, 460);
            oWnd.set_modal(true);
            return false;
        }
