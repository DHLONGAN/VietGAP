<%@ Page Title="" Language="C#" MasterPageFile="~/SiteMain.Master" AutoEventWireup="true" CodeBehind="User.aspx.cs" Inherits="Management.Sys.User" %>
...
<asp:ScriptManager ID="ScriptManager1" runat="server">
     ...
     <div id="ListLayer" style="border: 0px solid red; width:100%; overflow: hidden; 
        position: absolute;">
        <div class="TitleForm">
            <font size=6>DANH SÁCH NGƯỜI DÙNG</font>
        </div>
        ...
        <div>
            <asp:GridView ID="GV_User" runat="server" Width="100%" AutoGenerateColumns="false"
                CellPadding="4" GridLines="Both" CssClass="Border_GridView" ShowHeader="true"
                AllowSorting="True" onrowcommand="GV_User_RowCommand" >
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Tài khoản">
                        <ItemTemplate>                           
                                <div style="cursor: pointer" onclick="UserEdit('<%# Eval("UserKey")%>')">
                                <asp:Label ID="txtUserName_" Text='<%# Eval("UserName")%>' runat="server"></asp:Label>
                            </div>
                        </ItemTemplate>
                        <ItemStyle Width="150px" HorizontalAlign="Left" />
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="Được kích hoạt?">
                        <ItemTemplate>
                            <asp:ImageButton ID="cmdFrontPage2" CommandName="ItemFrontPage" ImageUrl='<%# String.Format("/Img/Icons/{0}.png", ConvertResult(Eval("Activate").ToString()))%>'
                                runat="server" CommandArgument='<%# Eval("UserKey")%>' />
                        </ItemTemplate>
...
<%--                    <asp:TemplateField HeaderText="Ngày hết hạn">
                        <ItemTemplate>
                           ...
                           <asp:Label ID="txtExpireDate" Text='<%# DateTime.Parse(Eval("ExpireDate").ToString()).ToString("dd/MM/yyyy")%>' runat="server"></asp:Label>
                        </ItemTemplate>
                        <ItemStyle Width="200px" HorizontalAlign="Left" />
                    </asp:TemplateField>--%>
                     <asp:TemplateField HeaderText="Thời gian đăng nhập cuối">
                        <ItemTemplate>
                             
                           <asp:Label ID="txtLastLoginDate" Text='<%# DateTime.Parse(Eval("LastLoginDate").ToString()).ToString("dd/MM/yyyy")%>' runat="server"></asp:Label>
                        </ItemTemplate>
                        <ItemStyle Width="200px" HorizontalAlign="Left" />
                    </asp:TemplateField>                    
<%--                    <asp:TemplateField HeaderText="Xã viên">
                    ...
                    <asp:TemplateField>
                    <HeaderTemplate>
                    <asp:Image ID="Image1" runat="server" ImageUrl="~\Img\Icons\add_one.png" style="cursor: pointer" onClick="UserEdit(0)"/>
                    </HeaderTemplate>
                        <ItemTemplate>
                            <asp:ImageButton ID="cmdDel" CommandName="ItemDel" OnCommand="GrDelete" ImageUrl="~\Img\Icons\Del.gif " runat="server" OnClientClick="javascript:return confirm('Bạn có chắc chắn xóa không?');"  CommandArgument='<%# Eval("UserKey")%>' />
                        </ItemTemplate>
                        <ItemStyle Width="20px" HorizontalAlign="Center" />
    ...
    <script type="text/javascript">
        ...
        function UserEdit(ID) {
            ...
            oWnd.show();
            oWnd.setUrl("User_Edit.aspx?Key=" + ID);
            // oWnd.maximize();
            oWnd.setSize(500, 450);
            oWnd.set_modal(true);
            return false;
        }
