<%@ Page Title="" Language="C#" MasterPageFile="~/SiteMain.Master" AutoEventWireup="true" CodeBehind="ProcessPlant.aspx.cs" Inherits="Management.Sys.ProcessPlant" %>
...
<asp:ScriptManager ID="ScriptManager1" runat="server">
     ...
     <div id="ListLayer" style="border: 0px solid red; width:100%; overflow: hidden; 
        position: absolute;">
        <div class="TitleForm">
            <font size=6>DANH SÁCH QUY TRÌNH</font>
        </div>
        ...
        <div style="padding-right: 20px">
            <asp:GridView ID="GV_List" runat="server" Width="100%" AutoGenerateColumns="false"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Tên quy trình">
                        <ItemTemplate>
                            <div style="cursor: pointer" onclick="Edit('<%# Eval("ProcessPlantKey")%>')">
                                <asp:Label ID="txtDatetime" Text='<%# Eval("ProcessPlantName")%>' runat="server"></asp:Label>
                            </div>
                        </ItemTemplate>
                    ...
                    <asp:TemplateField HeaderText="Loại quy trình">
                        <ItemTemplate>
                    ...
                    <asp:TemplateField>
                        <ItemTemplate>
                        <div style="cursor: pointer" onclick="Detail('<%# Eval("ProcessPlantKey")%>')">
                            <asp:Image ID="Edit" ImageUrl="~\Img\Icons\circle_more_detail-16.png" runat="server" />
                        </div>
                        </ItemTemplate>
                        <ItemStyle Width="20px" HorizontalAlign="Center" />
                    ...
                    <asp:TemplateField>
                        <ItemTemplate>
                        <div style="cursor: pointer" onclick="Edit_('<%# Eval("ProcessPlantKey")%>')">
                            <asp:Image ID="Edit" ImageUrl="~\Img\Icons\Edit.gif" runat="server" />
                        </div>
                        </ItemTemplate>
                        <ItemStyle Width="20px" HorizontalAlign="Center" />
                    ...
                    <asp:TemplateField>
                    <HeaderTemplate>
                    <asp:Image ID="Image1" runat="server" ImageUrl="~\Img\Icons\add_one.png" style="cursor: pointer" onClick="Edit_(0)"/>
                    </HeaderTemplate>
                        <ItemTemplate>
                            <asp:ImageButton ID="cmdDel" CommandName="ItemDel" OnCommand="GrDelete" ImageUrl="~\Img\Icons\Del.gif" runat="server" OnClientClick="javascript:return confirm('Bạn có chắc chắn xóa không?');"  CommandArgument='<%# Eval("ProcessPlantKey")%>' />
                        </ItemTemplate>
                        <ItemStyle Width="20px" HorizontalAlign="Center" />
    ...
    <script type="text/javascript">
        ...
        function Edit(ID) {
            ...
            oWnd.show();
            oWnd.setUrl("ProcessPlant_List.aspx?Key=" + ID);
            // oWnd.maximize();
        ...
        function Edit_(ID) {
            var oWnd = $find("<%= rwPesticide.ClientID %>");
            oWnd.show();
            oWnd.setUrl("ProcessPlant_Edit.aspx?Key=" + ID);
            // oWnd.maximize();
        ...
        function Detail(ID) {
            var oWnd = $find("<%= rwPesticide.ClientID %>");
            oWnd.show();
            oWnd.setUrl("ProcessPlant_Detail.aspx?Key=" + ID);
            // oWnd.maximize();
