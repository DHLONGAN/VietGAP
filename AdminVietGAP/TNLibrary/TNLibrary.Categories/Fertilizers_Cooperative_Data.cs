...
{
    public class Fertilizers_Cooperative_Data
    {
        public static DataTable GetList(int PageSize, int PageNumber, string Search, int CooperativeKey)
        {
            DataTable zTable = new DataTable();
            string zSQL = @"SELECT     dbo.PUL_Fertilizers.TradeName, dbo.PUL_Fertilizers_Cooperative.*
                        FROM         dbo.PUL_Fertilizers_Cooperative INNER JOIN
                      dbo.PUL_Fertilizers ON dbo.PUL_Fertilizers_Cooperative.FertilizersKey = dbo.PUL_Fertilizers.FertilizersKey WHERE CooperativeKey = @CooperativeKey";
            if (Search != "")
            {
                zSQL += " and dbo.PUL_Fertilizers.Trade_Name like N'%'+ @Search +'%'";
            }
            zSQL += " order by dbo.PUL_Fertilizers.TradeName";
        ...
        {
            int _Result = 0;
            string cmdText = @"SELECT  Count(*)  FROM         dbo.PUL_Fertilizers_Cooperative INNER JOIN
                      dbo.PUL_Fertilizers ON dbo.PUL_Fertilizers_Cooperative.FertilizersKey = dbo.PUL_Fertilizers.FertilizersKey WHERE CooperativeKey = @CooperativeKey";
            if (Search != "")
            {
                cmdText += " and dbo.PUL_Fertilizers.Trade_Name like N'%'+ @Search +'%'";
            }
