...
{
    public class ProvincesCities_Data
    {
        public static DataTable GetList(int PageSize, int PageNumber, string Search)
        {
            DataTable zTable = new DataTable();
            string zSQL = "SELECT  ProvincesCities_Key,ProvincesCities_ID,ProvincesCities_Name FROM PUL_ProvincesCities ";
            if (Search != "")
            {
                zSQL += " WHERE ProvincesCities_Name like N'%'+ @Search +'%'";
            }
        ...
        {
            int _Result = 0;
            string cmdText = @"SELECT  Count(*)  FROM  dbo.PUL_ProvincesCities";
            if (Search != "")
            {
                cmdText += " WHERE EquipmentName like N'%'+ @Search +'%'";
            }
