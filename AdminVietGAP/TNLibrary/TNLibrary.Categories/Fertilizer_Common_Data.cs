...
{
    public class Fertilizer_Common_Data
    {
        public static DataTable GetList(int PageSize, int PageNumber, string Search)
        {
            DataTable zTable = new DataTable();
            string zSQL = "SELECT  PUL_Fertilizer_Common.*, 1 As 'Loai' FROM PUL_Fertilizer_Common  ";
            if (Search != "")
            {
        ...
        {
            int _Result = 0;
            string cmdText = @"SELECT  Count(*)  FROM  dbo.PUL_Fertilizer_Common";
            if (Search != "")
            {
