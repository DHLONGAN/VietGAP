...
{
    public class Pollution_Data
    {
        public static DataTable GetList(int PageSize, int PageNumber, string Search)
        {
            DataTable zTable = new DataTable();
            string zSQL = "SELECT  * FROM PUL_Pollution ";
            if (Search != "")
            {
                zSQL += " WHERE PollutionName like N'%'+ @Search +'%'";
            }
        ...
        {
            int _Result = 0;
            string cmdText = @"SELECT  Count(*)  FROM  dbo.PUL_Pollution";
            if (Search != "")
            {
                cmdText += " WHERE PollutionName like N'%'+ @Search +'%'";
            }
